...
class _RaceSplitsAcessor(object):
    ...
    def _validate(race_result):
        assert isinstance(race_result, RaceResult), "RaceResult is needed for this accessor"
    ...
    def pick_athlete(self, identifier, by='BIB'):
        ...
        assert self._race_result.event.event_type in ('42k'), 'Until now, it only supports marathon splits race data.'
        ...
        if race_data.empty:
            raise ValueError('Identifier %s %s not found in the RaceResult set.' % (by_translated, identifier))
        ...
        distance_frame = pd.DataFrame.from_dict({'0k': 0, '5k': 5000, '10k': 10000, '15k': 15000,
                                 '20k': 20000, '25k': 25000, '30k': 30000, '35k': 35000, '40k': 40000,
                                 'nettime': 42195 , 'half': 21097}, orient='index', columns=['distance_meters'])
        race_data = race_data.join(distance_frame)

        race_data['distance_miles'] = race_data['distance_meters'].apply(lambda x : x*0.000621371).round(4)
