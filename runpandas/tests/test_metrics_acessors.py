...
@pytest.mark.parametrize("activity,column,index,expected", test_correct_distance_pos_data)
def test_metrics_corrected_distance(activity, column, index, expected):
    activity['distpos'] =  activity.compute.distance(correct_distance=True, to_special_column=False)
    assert activity[column].iloc[index] == expected
